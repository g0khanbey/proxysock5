...
# Check if UFW is active and open port 1100 if needed
if sudo ufw status | grep -q "Status: active"; then
    sudo ufw allow 1100/tcp
    sudo ufw allow 8100/tcp
fi

# Check if iptables is active and open port 1100 and 8100 if needed
if ! sudo iptables -L | grep -q "ACCEPT     tcp  --  anywhere             anywhere             tcp dpt:1100"; then
    sudo iptables -A INPUT -p tcp --dport 1100 -j ACCEPT
fi
if ! sudo iptables -L | grep -q "ACCEPT     tcp  --  anywhere             anywhere             tcp dpt:8100"; then
...
<VirtualHost *:8100>
    ...
    ProxyPass /socks5 http://localhost:1100/
    ProxyPassReverse /socks5 http://localhost:1100/
</VirtualHost>
EOF'
